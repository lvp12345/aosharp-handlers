...
using static SocialPetManager.SocialPetManager;
...
namespace SocialPetManager
{
    ...
    public static class SettingsController
    {
        ...
        private static void RegisterChatCommandIfNotRegistered()
        {
            ...
            {
                Chat.RegisterCommand("SocialPetManager", (string command, string[] param, ChatWindow chatWindow) =>
                {
                    try
                    {
                        settingsWindow = Window.Create(new Rect(50, 50, 300, 300), "Social Pet Manager", "Settings", WindowStyle.Default, WindowFlags.AutoScale);
                        settingsWindow.MoveTo(150, 150);

                        if (settingsWindow != null && !settingsWindow.IsVisible)
                        {
                            AppendSettingsTab("Social Pet Manager", settingsWindow);

                            settingsWindow.FindView("SocialPetManagerSettingWindow", out View SettingWindow);
